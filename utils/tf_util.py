...
Author: Charles R. Qi
Date: November 2016
Edited by: Yizhak Ben-Shabat, Mohammad Wasil
Date: February 2018, 2019
...
def get_3dmfv_sym(points, w, mu, sigma, sym_type='max', flatten=True):
    """
    Compute the 3d modified fisher vector (on the gpu using tf) given the gmm model parameters (w,mu,sigma) and 
    a set of points for classification network
...
def get_sess_conf(gpu_idx, limit_gpu=True):
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    config.allow_soft_placement = True
    config.log_device_placement = False
...
#it's based on scikit pca
def svd_flip(u,v, u_based_decision=True):
    if u_based_decision:
        # columns of u, rows of v
        max_abs_cols = tf.argmax(tf.abs(u), axis=0)
        signs = tf.sign(u[max_abs_cols, tf.range(u.shape[1])])
        u *= signs
        v *= signs[:, tf.newaxis]
    else:
        # rows of v, columns of u
        max_abs_rows = tf.argmax(tf.abs(v), axis=1)
        signs = tf.sign(v[tf.range(v.shape[0]), max_abs_rows])
        u *= signs
        v *= signs[:, tf.newaxis]
    return u, v

def pca_compress(points, n_components):
    # 2. Perform SVD and get the sigma value and get the sigma value
    s, u, v = tf.svd(points,full_matrices=False)
    v_adj =  tf.linalg.adjoint(v)
...
def normalize_points(points):
    ...
    principal_components = pca_compress(points, n_components)
    squared_length_principal_components = tf.multiply(principal_components, principal_components)
    length_principal_components = tf.sqrt(tf.reduce_sum(squared_length_principal_components, axis=1))

    R = principal_components / length_principal_components
    
    normalized_points = tf.transpose(tf.tensordot(R, tf.transpose(points), 1))

    return normalized_points

def normalize_batch_points(batch_points):
    axis = 0
    normalized_batch_points = tf.concat([tf.expand_dims(normalize_points(points), axis) for points in batch_points], axis)
    return normalized_batch_points
...
def get_bn_decay(batch, bn_init_decay, batch_size, bn_decay_decay_step, bn_decay_decay_rate, bn_decay_clip):
