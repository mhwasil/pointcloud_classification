...
def visualize_histogram(total_correct_class, total_seen_class,label_map, model, filepath):
    class_acc = []
    for pred,true in zip(total_correct_class, total_seen_class):
        norm_pred = pred/true
        class_acc.append(norm_pred)
    class_acc = np.asarray(class_acc)*100
    
    indices = np.argsort(class_acc)[::-1][:len(label_map)]
    class_acc = class_acc[indices]
    ...
    plt.ylabel("Accuracy")
    plt.bar(label_map, class_acc)
    plt.xticks(rotation=90)

    plt.savefig(filepath, format='pdf', dpi=300, bbox_inches='tight')

def visualize_confusion_matrix(y_true, y_pred, classes=None, normalize=False, 
                               cmap=cm.Greens, export=False, display=False, 
                               filename='confusion_mat', n_classes=40, acc_fontsize=3.5, 
                               title='Confusion Matrix', label_fontsize=5):
    """
    ...
    im = plt.imshow(conf_mat, cmap=cmap)
    ax = plt.gca()
    # Hide grid lines
    ax.grid(False)
    
    fig.colorbar(im, orientation="vertical", fraction=0.045, pad=0.05)
    
    #Write the labels for each row and column
    if classes is not None:
        tick_marks = np.arange(len(classes))
        plt.xticks(tick_marks, classes, rotation=90,  fontsize=label_fontsize)
    ...
    for i, j in itertools.product(range(conf_mat.shape[0]), range(conf_mat.shape[1])):
        plt.text(j, i, conf_mat[i, j],
                 horizontalalignment="center", fontsize=acc_fontsize,
