...
def shuffle_data(pointcloud, labels):
    ...
    return pointcloud[idx, ...], labels[idx], idx
...
def get_label_map(label_map_file):
    if os.path.isfile(label_map_file):
        label_map = {}
        with open(label_map_file, 'r') as infile:
            label_map = yaml.safe_load(infile)['label']
    else:
...
def load_h5(h5_filename, compensate=False, unify=False,
            load_image=False, load_mask=False, load_feature=False, num_feature=33,
            cloud_color=False):
    ...
    if cloud_color:
      ...
      pointcloud = f['pointcloud'][:,:,0:3]

    image = np.zeros((label.shape[0], 299,299, 3))
    mask = np.zeros((label.shape[0], 299,299, 3))
    feature = np.zeros((label.shape[0], 2048, 33))
    ...
    return (pointcloud,image,mask,feature,label)
...
#-----------------------------------Additional function for size independent 3d net----------#
#------- github.com/mhwasil ------#
...
def load_pickle_file_with_label(pickle_file_name, compressed=False, cloud_color=True):
    if compressed:
...
def scale_point_cloud2(batch_data, smin = 0.66, smax = 1.5):
    """ Randomly scale the point clouds to augument the dataset
        scale is per shape
        Input:
          BxNx3 array, original batch of point clouds
    ...
    for k in range(batch_data.shape[0]):
        ...
        scale_matrix = np.array([[sx, 0, 0],
                                 ...
                                 [0, 0, sz]])
        shape_pc = batch_data[k]
        scaled = np.dot(shape_pc.reshape((-1, 3)), scale_matrix)
        scaled_data.append(scaled)

    return np.asarray(scaled_data)

def translate_point_cloud2(batch_data, tval = 0.2):
    """ Randomly translate the point clouds to augument the dataset
        rotation is per shape based along up direction
        Input:
          BxNx3 array, original batch of point clouds
    ...
    for k in range(n_batches):
        n_points = batch_data[k].shape[0]
        translation = np.random.uniform(-tval, tval, size=[n_points,3])
        translated_batch.append(batch_data[k] + translation)
        
    return np.asarray(translated_batch)
...
def jitter_point_cloud(batch_data, sigma=0.01, clip=0.05):
    ...
    jittered_batch = []
    for k in range(batch_data.shape[0]):
        N, C = batch_data[k].shape
        jittered_points = np.clip(sigma * np.random.randn(N, C), -1*clip, clip)
        jittered_points += batch_data[k]
        jittered_batch.append(jittered_points)
        
    return (np.asarray(jittered_batch))